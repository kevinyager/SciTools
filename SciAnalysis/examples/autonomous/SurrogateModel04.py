...
class SurrogateModel(Base):
    ...
    def interpolate_gpcam(self, hps_guess=None, gp_method='global', pre_optimize=False, fill='pixelwise', gpcam_PATH=None, error_relative=None, renormalize_signals=True, hps_lock=None, convex_clip=False, **kwargs):
        ...
        gp = GPOptimizer(
                ...
                output_space_dimension=1, # The dimensionality of each signal (1=each signal is scalar)
                output_number=signals.shape[1], # Number of signals
                index_set_bounds=index_set_bounds, # Valid range for each parameter
        ...
        if gp_method is not None:
            ...
            gp.train_gp(hps_bounds,
                        likelihood_optimization_method=gp_method, # 'global', 'local'
                        likelihood_optimization_pop_size=20,
                        likelihood_optimization_tolerance=1e-4,
...
if __name__ == '__main__':
    ...
    kwargs = {
        ...
        'gp_method': None , # 'global', 'local', None (don't optimize)
        'pre_optimize': False ,
        'figsize': figsize ,
